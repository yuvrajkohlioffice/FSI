...
if(isset($_GET['action']) && !empty($_GET['action']) && $_GET['action']=='active')
{
	...
	if($capture==$postcapture && $capture!="" && $postcapture!="")
	{
		...
		$query="update firepoint set status='inactive' where sno='$unid_id'";
		$rs=mysql_query($query);
		
		if($rs)
		{
			echo "<script>location.href='firepoint-database.php?display=status'</script>";	
		}
		else
		{
			echo "<script>location.href='firepoint-database.php?error=incorrect'</script>";	
		}
	}
	else
	{
		echo "<p align='center'>Please Wait..</p>";
		echo "<script>location.href='firepoint-database.php?status=codeinvalid'</script>";
	}
}
...
if(isset($_GET['action']) && !empty($_GET['action']) && $_GET['action']=='inactive')
{
	...
	if($capture==$postcapture && $capture!="" && $postcapture!="")
	{
		...
		$query="update firepoint set status='active' where sno='$unid_id'";
		$rs=mysql_query($query);
	
		if($rs)
		{
			echo "<script>location.href='firepoint-database.php?display=update'</script>";	
		}
		else
		{
			echo "<script>location.href='firepoint-database.php?error=incorrect'</script>";	
...
if(isset($_POST['flag']) && !empty($_POST['flag']) && $_POST['flag']==1)
{
	...
	if($capture==$postcapture && $capture!="" && $postcapture!="")
	{
		...
		if($cou==0)
		{
			...
			if($rs)
			{
				echo "<script>location.href='firepoint-database.php?display=add'</script>";
			}
		}
		else
		{
			echo "<script>alert('User Name Already Exists in Database. Please choose another User Name.');location.href='firepoint-database.php';</script>";
		}
	}
	else
	{
...
if(isset($_GET['action']) && !empty($_GET['action']) && $_GET['action']=='delete_heading')
{
	...
	if($capture==$postcapture && $capture!="" && $postcapture!="")
	{
	    ...
	    $rs = mysql_query("delete from firepoint where sno='$unid_id'");    
		
		if($rs)
		{
			echo "<script>location.href='firepoint-database.php?display=delete'</script>";
		}
		else
		{
			echo "<script>location.href='firepoint-database.php?error=incorrect'</script>";
...
if(isset($_POST['action']) && !empty($_POST['action']) && $_POST['action']=='edit1')
{
	...
	if($capture==$postcapture && $capture!="" && $postcapture!="")
	{
		$f_date = date("Y-m-d",strtotime($_POST['fdate1']));
	
		$sql="update firepoint set fdate = '$f_date', longitude = '$_POST[longitude]', latitude = '$_POST[latitude]', district = '$_POST[district]', state = '$_POST[state]' where sno='$unid_id'";
...
function view()
{
	...
	$tbl_name="firepoint";		//your table name
	...
	*/
	...
	{
		$query = "SELECT COUNT(*) as num FROM $tbl_name where (fdate like '%$_GET[q]%' || district like '%$_GET[q]%' || state like '%$_GET[q]%')";
	}
	else if(isset($_GET['qdate']) && !empty($_GET['qdate']))
	{
		$query = "SELECT COUNT(*) as num FROM $tbl_name where fdate = '".date("Y-m-d", strtotime($_GET[qdate]))."'";
	}	
	else
	{
	...
	$targetpage = "firepoint-database.php"; 	//your file name  (the name of this file)
	...
	if(isset($_GET['q']) && !empty($_GET['q']))
	{
		$sql = "select * from $tbl_name where (fdate like '%$_GET[q]%' || district like '%$_GET[q]%' || state like '%$_GET[q]%') order by fdate desc LIMIT $start, $limit";
	}
	else if(isset($_GET['qdate']) && !empty($_GET['qdate']))
	{
		$sql = "select * from $tbl_name where  fdate = '".date("Y-m-d", strtotime($_GET[qdate]))."' order by fdate desc LIMIT $start, $limit";
	}	
	else
	{
		$sql = "select * from $tbl_name order by fdate desc LIMIT $start, $limit";
	}
			...
			<div class="container-fluid">
				...
				<div class="row-fluid">
					<div class="span12">
						...
						<form name="Search" action="firepoint-database.php" method="get">
						<input class="m-wrap" size="16" type="text" name="qdate" autocomplete="off" placeholder="Select Date" value="" style="width: 100px;" id="ui_date_picker_change_year_month"/>
						<select class="span6 m-wrap" name="q" style="width: 119px;">
						<option value="" selected>Select State</option>
							<?php
							$sql12="select distinct(state) from firepoint where state != '' order by state asc";
						...
						<form name="Search" action="firepoint-database.php" method="get">
						<input type="text" placeholder="date/district/state" value="<?php if(isset($_GET['q']) && !empty($_GET['q'])) { echo $_GET['q']; } ?>" required name="q">
				...
				<div class="row-fluid">
					<div class="span12">
						...
						?>
						 <form name="delete" action="multiple.php" onsubmit="return validate(this)" method="post"><br>
							  <input type="submit" value="Delete Selected" name="deleteval">
							  <input type="hidden" name="delpage" value="<?php if( isset($_GET['page']) ) { echo $_GET['page'];}?>">
						<div class="portlet box blue">
							<div class="portlet-title">
								<div class="caption"><i class="icon-cogs"></i>Manage Firepoints</div>
								<div class="tools">
							...
							<div class="portlet-body">
								<table class="table table-hover">
									<thead>
										<tr>
											<th># <input type="checkbox" name="select_all" value="" id="selecctall"></th>
											<th>Fire Date</th>
											<th>Longitude</th>
											<th>Latitude</th>
											<th>SOI</th>
									...
									?>
										<tr>
											<td><input type="checkbox" class="checkbox1" name="selectval[]" value="<?php echo $row_mainmenu['sno'];?>">&nbsp; <?php echo $sno; ?></td>
											<td><?php echo $row_mainmenu['fdate'];?></td>
											<td><?php echo $row_mainmenu['longitude'];?></td>
											<td><?php echo $row_mainmenu['latitude'];?></td>
											<td><?php echo $row_mainmenu['soi'];?></td>
											<td><?php echo $row_mainmenu['district'];?></td>
											<td><?php echo $row_mainmenu['state'];?></td>																	
											<td>
											<a href="firepoint-database.php?action=<?php echo $row_mainmenu['status']?>&sno=<?php echo $row_mainmenu['sno']?>&log=<?php echo $_SESSION['capture_code'];?>">
											<?php
											...
											<td width="15">
											<a href="firepoint-database.php?sno=<?php echo $row_mainmenu['sno'];?>&action=<?php echo 'edit_heading'?>&log=<?php echo $_SESSION['capture_code'];?>">
											<img border="0" src="images/edit.png" width="20" height="20" alt="Edit" name="Edit"></a></td>
											<td width="15"><input type="image" img border="0" src="images/delete.png" width="20" height="20" onclick="var a=confirm('Are You sure delete this record from database ?');if(a==true){location.href='firepoint-database.php?sno=<?php echo $row_mainmenu['sno']?>&action=<?php echo 'delete_heading'?>&log=<?php echo $_SESSION['capture_code'];?>';}" alt="Delete" name="delete"></td>										   
										</tr>
										<?php
										}
										?>
		   ...
		   jQuery('#selecctall').click(function(event) {  //on click 
	        if(this.checked) { // check select status
	            jQuery('.checkbox1').each(function() { //loop through each checkbox
	                ...
	                jQuery('.checker span').addClass("checked");               
	            });
	        }else{
	            jQuery('.checkbox1').each(function() { //loop through each checkbox
