...
if(isset($_GET['action']) && !empty($_GET['action']) && $_GET['action']=='active')
{
	...
	if($capture==$postcapture && $capture!="" && $postcapture!="")
	{
		...
		$query="update departmental_circulars set status='inactive' where ser='$unid_id'";
	...
	$_SESSION['capture_code']=$security_code;
		if($rs)
		{
			echo "<script>location.href='circulars.php?display=status'</script>";	
		}
		else
		{
			echo "<script>location.href='circulars.php?error=incorrect'</script>";	
		}
	}
	else
	{
		echo "<p align='center'>Please Wait..</p>";
		echo "<script>location.href='circulars.php?status=codeinvalid'</script>";
	}
}
...
if(isset($_GET['action']) && !empty($_GET['action']) && $_GET['action']=='inactive')
{
	...
	if($capture==$postcapture && $capture!="" && $postcapture!="")
	{
		$query="update departmental_circulars set status='active' where ser='$unid_id'";
		$rs=mysql_query($query);
		$md5_hash = md5(rand(0,999)); 
	$security_code = substr($md5_hash, 15, 6); 
	$_SESSION['capture_code']=$security_code;
		if($rs)
		{
			echo "<script>location.href='circulars.php?display=update'</script>";	
		}
		else
		{
			echo "<script>location.href='circulars.php?error=incorrect'</script>";	
...
if(isset($_POST['flag']) && !empty($_POST['flag']) && $_POST['flag']==1)
{
	...
	if($capture==$postcapture && $capture!="" && $postcapture!="")
	{
		...
		$img_response = sh_upload_file("txtbanner", "image", "../uploads/circulars/", "cir_".$ran);
		if($img_response)
		{
			$fname = $img_response;
		}
		...
		$doc_response = sh_upload_file("txtfile", "file", "../uploads/circulars/", "cir_".$ran);
		if($doc_response)
		{
			$iname = $doc_response;
		}
		...
		$query="insert into departmental_circulars set newsdate='$news_date', title='".filter_xss($_POST['txtheading'])."',description='$desc',friendly_url='".filter_xss($_POST['friendly_url'])."', link='".filter_xss($_POST['url'])."', target='$_POST[target]',status='active', extension='$iname',image='$fname',crdate='$crdate',sorder='".filter_xss($_POST['txtposition'])."'";
	...
	$_SESSION['capture_code']=$security_code;
		if($rs)
		{
			echo "<script>location.href='circulars.php?display=add'</script>";
		}
		else
		{
			echo "<script>location.href='circulars.php?error=incorrect'</script>";
...
if(isset($_GET['action']) && !empty($_GET['action']) && $_GET['action']=='delete_heading')
{
	...
	if($capture==$postcapture && $capture!="" && $postcapture!="")
	{
		...
		$query="delete from departmental_circulars where ser='$unid_id'";
		$rs=mysql_query($query);
			$md5_hash = md5(rand(0,999)); 
	$security_code = substr($md5_hash, 15, 6); 
	$_SESSION['capture_code']=$security_code;
		if($rs)
		{
			echo "<script>location.href='circulars.php?display=delete'</script>";
		}
		else
		{
			echo "<script>location.href='circulars.php?error=incorrect'</script>";
...
if(isset($_POST['action']) && !empty($_POST['action']) && $_POST['action']=='edit1')
{
	...
	if($capture==$postcapture && $capture!="" && $postcapture!="")
	{
		...
		$query="update departmental_circulars set newsdate='$news_date', title='".filter_xss($_POST['txtheading'])."',description='$desc',friendly_url='".filter_xss($_POST['friendly_url'])."', link='".filter_xss($_POST['url'])."', target='$_POST[target]',sorder='".filter_xss($_POST['txtposition'])."'";
...
function view()
{
	...
	$tbl_name="departmental_circulars";		//your table name
	...
	*/
	...
	$targetpage = "circulars.php"; 	//your file name  (the name of this file)
	...
	$sql = "select * from departmental_circulars order by ser desc LIMIT $start, $limit";
				...
				<div class="row-fluid">
					<div class="span12">						     
						<h3 class="page-title">
							...
							<small>Add/Edit Circulars</small>
						</h3>
						<ul class="breadcrumb">
							<li>
								<i class="icon-home"></i>
					...
					<div class="span12">
						...
						<div class="portlet box blue">
							<div class="portlet-title">
								<div class="caption"><i class="icon-cogs"></i>Departmental Circulars</div>
								<div class="tools">
								...
								<table class="table table-hover">
									...
									?>
										<tr>
											...
											<td>
											<a href="circulars.php?action=<?php echo $row_content['status']?>&sno=<?php echo $row_content['ser']?>&log=<?php echo $_SESSION['capture_code'];?>">
											<?php
											...
											<td>
											<a href="circulars.php?sno=<?php echo $row_content['ser'];?>&action=<?php echo 'edit_heading'?>&log=<?php echo $_SESSION['capture_code'];?>">
											<img border="0" src="images/edit.png" width="20" height="20" alt="Edit" name="Edit"></a></td>
											<td><input type="image" img border="0" src="images/delete.png" width="20" height="20" onclick="var a=confirm('Are You sure delete this record from database ?');if(a==true){location.href='circulars.php?sno=<?php echo $row_content['ser']?>&action=<?php echo 'delete_heading'?>&log=<?php echo $_SESSION['capture_code'];?>';}" alt="Delete" name="delete"></td>
				...
				<div class="row-fluid">
					<div class="span12">
						...
						<div class="portlet box purple">
							<div class="portlet-title">
								<div class="caption"><i class="icon-reorder"></i>Add Circular</div>
								<div class="tools">
									<a href="javascript:;" class="collapse"></a>
									<!--a href="#portlet-config" data-toggle="modal" class="config"></a-->
									<a href="javascript:;" class="reload"></a>
							...
							<div class="portlet-body form">
								<!-- BEGIN FORM-->
								<form action="circulars.php?action=category" enctype="multipart/form-data" id="form_page" name="form_page" class="form-horizontal" method="post">
									
									<div class="control-group">
										<label class="control-label">Circular Date</label>
										<div class="controls">
...
function edit()
{
	...
	{	
		...
		$query=mysql_query("select * from departmental_circulars where ser='$contentid'");
			...
			<div class="container-fluid">
				...
				<div class="row-fluid">
					...
					<div class="span12">
						...
						<div class="portlet box purple">
							<div class="portlet-title">
								<div class="caption"><i class="icon-reorder"></i>Edit Circular - <b>"<?php echo $row_content['title'];?>"</b></div>
								<div class="tools">
									<a href="javascript:;" class="collapse"></a>
									<!--a href="#portlet-config" data-toggle="modal" class="config"></a-->
									<a href="javascript:;" class="reload"></a>
							...
							<div class="portlet-body form">
								<!-- BEGIN FORM-->
								<form action="circulars.php?action=category" enctype="multipart/form-data" id="form_page" name="editform_page" class="form-horizontal" method="post">
									...
									<div class="control-group">
										...
										<div class="controls">
											...
											<?php
											if($row_content['image']!='' && file_exists("../uploads/circulars/".$row_content['image']))
											{
											?>
											<a href="../uploads/circulars/<?php echo $row_content['image']?>" title="View Attachment" target="_blank">
									...
									<div class="control-group">
										...
										<div class="controls">
											...
											<?php
													if($row_content['extension']!='' && file_exists("../uploads/circulars/".$row_content['extension']))
													{
													?>
													<a href="../uploads/circulars/<?php echo $row_content['extension']?>" title="View Attachment" target="_blank">
													<font color="#6A9D00" size="2" face="Trebuchet MS">view attachment</font></a>
													<?php
													}
													?>
...
<?php
	...
	{
		echo "<script>location.href='circulars.php';</script>";
	}
}
?>
