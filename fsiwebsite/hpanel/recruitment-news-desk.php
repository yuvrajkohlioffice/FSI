...
if(isset($_GET['action']) && !empty($_GET['action']) && $_GET['action']=='active')
{
	...
	if($capture==$postcapture && $capture!="" && $postcapture!="")
	{
		...
		$query="update recruitment_news set status='inactive' where ser='$unid_id'";
	...
	$_SESSION['capture_code']=$security_code;
		if($rs)
		{
			echo "<script>location.href='recruitment-news-desk.php?display=status'</script>";	
		}
		else
		{
			echo "<script>location.href='recruitment-news-desk.php?error=incorrect'</script>";	
		}
	}
	else
	{
		echo "<p align='center'>Please Wait..</p>";
		echo "<script>location.href='recruitment-news-desk.php?status=codeinvalid'</script>";
	}
}
...
if(isset($_GET['action']) && !empty($_GET['action']) && $_GET['action']=='inactive')
{
	...
	if($capture==$postcapture && $capture!="" && $postcapture!="")
	{
		...
		$query="update recruitment_news set status='active' where ser='$unid_id'";
		$rs=mysql_query($query);
		$md5_hash = md5(rand(0,999)); 
	$security_code = substr($md5_hash, 15, 6); 
	$_SESSION['capture_code']=$security_code;
		if($rs)
		{
			echo "<script>location.href='recruitment-news-desk.php?display=update'</script>";	
		}
		else
		{
			echo "<script>location.href='recruitment-news-desk.php?error=incorrect'</script>";	
...
if(isset($_POST['flag']) && !empty($_POST['flag']) && $_POST['flag']==1)
{
	...
	if($capture==$postcapture && $capture!="" && $postcapture!="")
	{
		...
		$query="insert into recruitment_news set newsdate='$news_date', title='".filter_xss($_POST['txtheading'])."',description='$desc', link='".filter_xss($_POST['url'])."', target='$_POST[target]',status='active', extension='$iname',image='$fname',crdate='$crdate',sorder='".filter_xss($_POST['txtposition'])."'";
	...
	$_SESSION['capture_code']=$security_code;
		...
		if($rs)
		{
			...
			$url = "recruitment-news-desk.php?display=add"; 
...
if(isset($_GET['action']) && !empty($_GET['action']) && $_GET['action']=='delete_heading')
{
	...
	if($capture==$postcapture && $capture!="" && $postcapture!="")
	{
		...
		$query="delete from recruitment_news where ser='$unid_id'";
		$rs=mysql_query($query);
			$md5_hash = md5(rand(0,999)); 
	$security_code = substr($md5_hash, 15, 6); 
	$_SESSION['capture_code']=$security_code;
		if($rs)
		{
			echo "<script>location.href='recruitment-news-desk.php?display=delete'</script>";
		}
		else
		{
			echo "<script>location.href='recruitment-news-desk.php?error=incorrect'</script>";
...
if(isset($_POST['action']) && !empty($_POST['action']) && $_POST['action']=='edit1')
{
	...
	if($capture==$postcapture && $capture!="" && $postcapture!="")
	{
		...
		$query="update recruitment_news set newsdate='$news_date', title='".filter_xss($_POST[txtheading])."',description='$desc',friendly_url='".filter_xss($_POST[friendly_url])."', link='".filter_xss($_POST[url])."', target='$_POST[target]',sorder='".filter_xss($_POST[txtposition])."'";
	...
	$_SESSION['capture_code']=$security_code;
		if($rs)
		{
			...
			$url = "recruitment-news-desk.php?display=update"; 
...
function view()
{
	...
	$tbl_name="recruitment_news";		//your table name
	...
	*/
	...
	$targetpage = "recruitment-news-desk.php"; 	//your file name  (the name of this file)
	$limit = 10; 	
	$page = 0;
	if(isset($_GET['page']))	
	...
	$sql = "select * from recruitment_news order by ser desc LIMIT $start, $limit";
			...
			<div class="container-fluid">
				...
				<div class="row-fluid">
					<div class="span12">						     
						<h3 class="page-title">
							Recruitment News
							<small>Add/Edit Website Recruitment Notice</small>
						</h3>
						<ul class="breadcrumb">
							<li>
								<i class="icon-home"></i>
							...
							<li><a href="#">Recruitment Notice</a></li>
						</ul>
					</div>
				</div>
				<!-- END PAGE HEADER-->
					...
					<div class="span12">
						...
						<div class="portlet box blue">
							<div class="portlet-title">
								<div class="caption"><i class="icon-cogs"></i>Recruitment Notices</div>
								<div class="tools">
								...
								<table class="table table-hover">
									...
									?>
										<tr>
											...
											<td>
											<a href="recruitment-news-desk.php?action=<?php echo $row_content['status']?>&sno=<?php echo $row_content['ser']?>&log=<?php echo $_SESSION['capture_code'];?>">
											<?php
											...
											<td>
											<a href="recruitment-news-desk.php?sno=<?php echo $row_content['ser'];?>&action=<?php echo 'edit_heading'?>&log=<?php echo $_SESSION['capture_code'];?>">
											<img border="0" src="images/edit.png" width="20" height="20" alt="Edit" name="Edit"></a></td>
											<td><input type="image" img border="0" src="images/delete.png" width="20" height="20" onclick="var a=confirm('Are You sure delete this record from database ?');if(a==true){location.href='recruitment-news-desk.php?sno=<?php echo $row_content['ser']?>&action=<?php echo 'delete_heading'?>&log=<?php echo $_SESSION['capture_code'];?>';}" alt="Delete" name="delete"></td>
											<td>
											<?php
				...
				<div class="row-fluid">
					<div class="span12">
						...
						<div class="portlet box purple">
							<div class="portlet-title">
								<div class="caption"><i class="icon-reorder"></i>Add Recruitment Notice</div>
								<div class="tools">
									<a href="javascript:;" class="collapse"></a>
									<!--a href="#portlet-config" data-toggle="modal" class="config"></a-->
									<a href="javascript:;" class="reload"></a>
							...
							<div class="portlet-body form">
								<!-- BEGIN FORM-->
								<form action="recruitment-news-desk.php?action=category" enctype="multipart/form-data" id="form_page" name="form_page" class="form-horizontal" method="post">
...
function edit()
{
	...
	{	
		...
		$query=mysql_query("select * from recruitment_news where ser='$contentid'");
				...
				<div class="row-fluid">
					<div class="span12">
						...
						<ul class="breadcrumb">
							...
							<li>
								<a href="recruitment-news-desk.php">Pages</a>
								<span class="icon-angle-right"></span>
							</li>
				...
				<div class="row-fluid">
					...
					<div class="span12">
						...
						<div class="portlet box purple">
							...
							<div class="portlet-body form">
								<!-- BEGIN FORM-->
								<form action="recruitment-news-desk.php?action=category" enctype="multipart/form-data" id="form_page" name="editform_page" class="form-horizontal" method="post">
									
									<div class="control-group">
										<label class="control-label">News Date</label>
										<div class="controls">
											<input class="m-wrap" size="16" type="text" name="newsdate" autocomplete="off" value="<?php echo date('d-M-Y',strtotime($row_content['newsdate']));?>" id="ui_date_picker_change_year_month"/>
										</div>
									</div>
									<div class="control-group">
										<label class="control-label">News Heading<span class="required">*</span></label>
										<div class="controls">
											<input type="text" name="txtheading" value="<?php echo $row_content['title'];?>" value="<?php echo $row_mainmenu['name'];?>" data-required="1" class="span6 m-wrap"/>
										</div>
									</div>
									<div class="control-group">
										<label class="control-label">Menu Slug / Friendly URL&nbsp;&nbsp;</label>
...
<?php
	...
	{
		echo "<script>location.href='recruitment-news-desk.php';</script>";
	}
}
?>
