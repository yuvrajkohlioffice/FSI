...
if(isset($_GET['action']) && !empty($_GET['action']) && $_GET['action']=='active')
{
	...
	if($capture==$postcapture && $capture!="" && $postcapture!="")
	{
		...
		$query="update services set status='inactive' where ser='$unid_id'";
		...
		if($rs)
		{
			echo "<script>location.href='services.php?display=status'</script>";	
		}
		else
		{
			echo "<script>location.href='services.php?error=incorrect'</script>";	
		}
	}
	else
	{
		echo "<p align='center'>Please Wait..</p>";
		echo "<script>location.href='services.php?status=codeinvalid'</script>";
	}
}
...
if(isset($_GET['action']) && !empty($_GET['action']) && $_GET['action']=='inactive')
{
	...
	if($capture==$postcapture && $capture!="" && $postcapture!="")
	{
		...
		$query="update services set status='active' where ser='$unid_id'";
		$rs=mysql_query($query);
		$md5_hash = md5(rand(0,999)); 
		$security_code = substr($md5_hash, 16, 16); 
		$_SESSION['capture_code']=$security_code;
		if($rs)
		{
			echo "<script>location.href='services.php?display=update'</script>";	
		}
		else
		{
			echo "<script>location.href='services.php?error=incorrect'</script>";	
...
if(isset($_GET['action']) && !empty($_GET['action']) && $_GET['action']=='delete_heading')
{
	...
	if($capture==$postcapture && $capture!="" && $postcapture!="")
	{
		...
		$query="delete from services where ser='$unid_id'";
		$rs=mysql_query($query);
		$md5_hash = md5(rand(0,999)); 
		$security_code = substr($md5_hash, 16, 16); 
		$_SESSION['capture_code']=$security_code;
		if($rs)
		{
			echo "<script>location.href='services.php?display=delete'</script>";
		}
		else
		{
			echo "<script>location.href='services.php?error=incorrect'</script>";
...
if(isset($_POST['action']) && !empty($_POST['action']) && $_POST['action']=='edit1')
{
	...
	if($capture==$postcapture && $capture!="" && $postcapture!="")
	{
		...
		$query="update services set postdate='$news_date', primeArea = '$_POST[primeArea]', title='".filter_xss(strip_tags($_POST[txtheading]))."',description='$desc',friendly_url='".filter_xss(strip_tags($_POST[friendly_url]))."', link='".$url."', target='".filter_xss(strip_tags($_POST[target]))."',sorder='".filter_xss($_POST[txtposition])."'";
		...
		if($rs)
		{
			...
			$url = "services.php?display=update"; 
...
function view()
{
	...
	*/
	...
	$sql = "select * from services order by ser desc LIMIT $start, $limit";
								...
								<table class="table table-hover">
									...
									?>
										<tr>
											...
											<img border=0 src="images/down.jpg" title="<?echo $row_content['image']?>" alt="View Banner"></a>
											<?
											}
											?>		
											</td>
											<td>
											<a href="services.php?action=<?php echo $row_content['status']?>&sno=<?php echo $row_content['ser']?>&log=<?php echo $_SESSION['capture_code'];?>">
											<?php
											...
											<td>
											<a href="services.php?sno=<?php echo $row_content['ser'];?>&action=<?php echo 'edit_heading'?>&log=<?php echo $_SESSION['capture_code'];?>">
											<img border="0" src="images/edit.png" width="20" height="20" alt="Edit" name="Edit"></a></td>
											<td><input type="image" img border="0" src="images/delete.png" width="20" height="20" onclick="var a=confirm('Are You sure delete this record from database ?');if(a==true){location.href='services.php?sno=<?php echo $row_content['ser']?>&action=<?php echo 'delete_heading'?>&log=<?php echo $_SESSION['capture_code'];?>';}" alt="Delete" name="delete"></td>
											<td>
											<?php
					...
					<div class="span12">
						...
						<div class="portlet box purple">
							...
							<div class="portlet-body form">
								<!-- BEGIN FORM-->
								<form action="services.php?action=category" enctype="multipart/form-data" id="form_page" name="form_page" class="form-horizontal" method="post">
...
function edit()
{
	...
	{	
		...
		$query=mysql_query("select * from services where ser='$contentid'");
			...
			<div class="container-fluid">
				...
				<div class="row-fluid">
					...
					<div class="span12">
						...
						<div class="portlet box purple">
							<div class="portlet-title">
								<div class="caption"><i class="icon-reorder"></i>Edit Content - <b>"<?php echo filter_xss($row_content['title']);?>"</b></div>
								<div class="tools">
									<a href="javascript:;" class="collapse"></a>
									<!--a href="#portlet-config" data-toggle="modal" class="config"></a-->
									<a href="javascript:;" class="reload"></a>
							...
							<div class="portlet-body form">
								<!-- BEGIN FORM-->
								<form action="services.php?action=category" enctype="multipart/form-data" id="form_page" name="editform_page" class="form-horizontal" method="post">
									<div class="control-group">
										<label class="control-label">Show in Prime Position<span class="required">*</span></label>
										<div class="controls">
											<select class="span6 m-wrap" name="primeArea">
