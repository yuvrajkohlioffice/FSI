...
if(isset($_GET['action']) && !empty($_GET['action']) && $_GET['action']=='active')
{
	...
	{		
		$query="update top_quick_menu set status='inactive' where sno='$unid_id'";
		...
		if($rs)
		{
			echo "<script>location.href='top-quick-menu.php?display=status'</script>";	
		}
		else
		{
			echo "<script>location.href='top-quick-menu.php?error=incorrect'</script>";	
		}
	}
	else
	{
...
if(isset($_GET['action']) && !empty($_GET['action']) && $_GET['action']=='inactive')
{
	...
	if($capture==$postcapture && $capture!="" && $postcapture!="")
	{	
		$query="update top_quick_menu set status='active' where sno='$unid_id'";
		$rs=mysql_query($query);
		$md5_hash = md5(rand(0,999)); 
		$security_code = substr($md5_hash, 16, 16); 
		$_SESSION['capture_code']=$security_code;
		if($rs)
		{
			echo "<script>location.href='top-quick-menu.php?display=update'</script>";	
		}
		else
		{
			echo "<script>location.href='top-quick-menu.php?error=incorrect'</script>";	
...
if(isset($_GET['action']) && !empty($_GET['action']) && $_GET['action']=='yes')
{
	...
	{	
		...
		$query="update top_quick_menu set home='no' where sno='$unid_id'";
		$rs=mysql_query($query);
		$md5_hash = md5(rand(0,999)); 
		$security_code = substr($md5_hash, 16, 16); 
		$_SESSION['capture_code']=$security_code;
...
if(isset($_GET['action']) && !empty($_GET['action']) && $_GET['action']=='no')
{
	...
	{	
		...
		$query="update top_quick_menu set home='yes' where sno='$unid_id'";
		$rs=mysql_query($query);
		$md5_hash = md5(rand(0,999)); 
		$security_code = substr($md5_hash, 16, 16); 
		$_SESSION['capture_code']=$security_code;
...
if(isset($_POST['flag']) && !empty($_POST['flag']) && $_POST['flag']==1)
{
	...
	{	
		...
		$query="insert into top_quick_menu(websitetype,name,status,position,otherurl,home,side_display,target) values('".filter_xss($_POST['website_type'])."','".filter_xss(strip_tags($_POST['txtheading']))."','active','".filter_xss($_POST['txtposition'])."','".$url."','".filter_xss($_POST['home'])."','".filter_xss($_POST['side_display'])."','".filter_xss($_POST['target'])."')";
		...
		if($rs)
		{
			echo "<script>location.href='top-quick-menu.php?display=add'</script>";
		}
		else
		{
			echo "<script>location.href='top-quick-menu.php?error=incorrect'</script>";
...
if(isset($_GET['action']) && !empty($_GET['action']) && $_GET['action']=='delete_heading')
{
	...
	if($capture==$postcapture && $capture!="" && $postcapture!="")
	{		
		$query="delete from top_quick_menu where sno='$unid_id'";
		$rs=mysql_query($query);
		$md5_hash = md5(rand(0,999)); 
		$security_code = substr($md5_hash, 16, 16); 
		$_SESSION['capture_code']=$security_code;
		if($rs)
		{
			echo "<script>location.href='top-quick-menu.php?display=delete'</script>";
		}
		else
		{
			echo "<script>location.href='top-quick-menu.php?error=incorrect'</script>";
...
if(isset($_POST['action']) && !empty($_POST['action']) && $_POST['action']=='edit1')
{
	...
	{	
		...
		$query="update top_quick_menu set websitetype='".filter_xss($_POST['website_type'])."', name='".filter_xss(strip_tags($_POST['txtheading']))."',otherurl='".$url."',target='".filter_xss($_POST['target'])."',home='".filter_xss($_POST['home'])."', side_display='".filter_xss($_POST['side_display'])."', position='".filter_xss($_POST['txtposition'])."' where sno='$unid_id'";
		...
		<div class="page-content">
			...
			<div class="container-fluid">
				...
				<div class="row-fluid">
					<div class="span12">						     
						<h3 class="page-title">
							Website Top Quick Menu
							<small>Add Website Top Quick Menu</small>
						</h3>
						<ul class="breadcrumb">
							<li>
								<i class="icon-home"></i>
							...
							<li><a href="#">Manage Top Quick Navigation</a></li>
						</ul>
					</div>
				</div>
				<!-- END PAGE HEADER-->
						...
						<div class="portlet box blue">
							...
							<div class="portlet-body">
								<table class="table table-hover">
									...
									<tbody>
									<?php
									$res_mainmenu=mysql_query("select * from top_quick_menu order by position asc");
									...
									?>
										<tr>
											...
											<td>
											<a href="top-quick-menu.php?action=<?php echo $row_mainmenu['status']?>&sno=<?php echo $row_mainmenu['sno']?>&log=<?php echo $_SESSION['capture_code'];?>">
											<?php
											...
											<td>
											<a href="top-quick-menu.php?sno=<?php echo $row_mainmenu['sno'];?>&action=<?php echo 'edit_heading'?>&log=<?php echo $_SESSION['capture_code'];?>">
											<img border="0" src="images/edit.png" width="20" height="20" alt="Edit" name="Edit"></a></td>
											<td><input type="image" img border="0" src="images/delete.png" width="20" height="20" onclick="var a=confirm('Are You sure delete this record from database ?');if(a==true){location.href='top-quick-menu.php?sno=<?php echo $row_mainmenu[sno]?>&action=<?php echo 'delete_heading'?>&log=<?php echo $_SESSION['capture_code'];?>';}" alt="Delete" name="delete"></td>
										</tr>
										<?php
										}
										?>
					...
					<div class="span12">
						...
						<div class="portlet box purple">
							...
							<div class="portlet-body form">
								<!-- BEGIN FORM-->
								<form action="top-quick-menu.php?action=category" id="form_main_menu" class="form-horizontal" method="post">								
...
function edit()
{
	...
	{	
		...
		$query=mysql_query("select * from top_quick_menu where sno='$menuid'");
				...
				<div class="row-fluid">
					<div class="span12">
						...
						<ul class="breadcrumb">
							...
							<li>
								<a href="top-quick-menu.php">Menu Master's</a>
								<span class="icon-angle-right"></span>
							</li>
				...
				<div class="row-fluid">
					...
					<div class="span12">
						...
						<div class="portlet box purple">
							...
							<div class="portlet-body form">
								<!-- BEGIN FORM-->
								<form action="top-quick-menu.php" id="form_main_menu" class="form-horizontal" method="post">									
									<div class="control-group">
										<label class="control-label">Display in Side Bar<span class="required">*</span></label>
										<div class="controls">
											<select class="span6 m-wrap" name="side_display">
...
<?php
	...
	{
		echo "<script>location.href='top-quick-menu.php';</script>";
	}
}
?>
