...
if(isset($_GET['pgID']) && !empty($_GET['pgID']) && $common->use_friendly_urls)
{
	...
	if(!empty($exp[0]))
	{
		...
		{
			if($page_type=="page")
				$seo_title=mysql_fetch_array(mysql_query("select name  from heading where status='active' and sno='$category_id' and websitetype='hindi'"));
			else if($page_type=="subpage")
				$seo_title=mysql_fetch_array(mysql_query("select subhead as name from sub_heading where status='active' and sno='$category_id' and websitetype='hindi'"));
			else if($page_type=="subsubpage")
				$seo_title=mysql_fetch_array(mysql_query("select subhead as name from sub_sub_heading where status='active' and sno='$category_id' and websitetype='hindi'"));	
...
if(isset($_GET['pgID']) && $_GET['pgID']!="" || $page_type =="page"  || $page_type =="subpage" || $page_type =="bottom" )
{
	...
	{
		$query="select * from link_page where (heading='$allid' or friendly_url='$category_id') and pagehead='m' and status='active' and websitetype='hindi'";
		$head="select name, sno, yearwisedata from heading where (sno='$allid' || seo_url='$category_id') and status='active' and websitetype='hindi'";	
		$pageh="i";
		$PageType = "m";
		$breadpageType = "main";
	}
	...
	{
		$query="select * from link_page where (heading='$allid' or friendly_url='$category_id') and pagehead='s' and status='active' and websitetype='hindi'";
		$head="select subhead, heading, sno, yearwisedata from sub_heading where (sno='$allid' || seo_url='$category_id') and status='active' and websitetype='hindi'";
		$pageh="i";
		$PageType = "s";	
		$breadpageType = "sub";
	}
	if($exp[0]=="sub")
	{
		$query="select * from link_page where heading='$allid' and pagehead='i' and status='active' and websitetype='hindi'";
		$head="select subhead,heading from sub_sub_heading where (sno='$allid' || friendly_url='$category_id') and status='active' and websitetype='hindi'";
		$pageh="i";
	...
	{
		$query="select * from link_page where heading='$allid' and pagehead='q' and status='active' and websitetype='hindi'";
		$head="select name,sno from quick_menu where (sno='$allid' || friendly_url='$category_id') and status='active' and websitetype='hindi'";
		$pageh="q";	
	}
	else if($exp[0]=="tp")
	{
		$query="select * from link_page where heading='$allid' and pagehead='k' and status='active' and websitetype='hindi'";
		$head="select name,sno from top_quick_menu where (sno='$allid' || friendly_url='$category_id') and status='active' and websitetype='hindi'";
		$pageh="q";	
	}

	else if($exp[0]=="bo" || $page_type=="bottom")
	{
		$query="select * from link_page where (heading='$allid' or friendly_url='$category_id') and pagehead='b' and status='active' and websitetype='hindi'";
		$head="select name from bottom_menu where (sno='$allid' || otherurl='$category_id') and status='active' and websitetype='hindi'";		
	}
	...
	if(isset($_GET['ctype']) && $_GET['ctype']!="" || $page_type == "subsubpage" )
	{
		...
		{	
			$query="select * from link_page where ( heading='$allidsub' || friendly_url='$category_id') and pagehead='i' and status='active' and websitetype='hindi'";
			
			$head_sub="select subhead,heading from sub_sub_heading where (sno='$allidsub' || otherurl='$category_id') and status='active' and websitetype='hindi'";			
...
<section id="landscaping-design-gardener">
		...
		if( $head )
		{
            	...
            	$query_sub = mysql_query("select * from sub_heading where status='active' and heading='$row_head[heading]' and websitetype='hindi' order by position");
                ...
                <div><img src="../img/ff.jpg" alt=""></div>
                    ...
                    <?php	
                    $mainHeadName = mysql_fetch_array( mysql_query( "select name from heading where sno = '$row_head[heading]' and websitetype='hindi'"));
		    ...
		    {
                $general_head=$row_details['package_page'];
               
				
            	$query_sub = mysql_query("select * from sub_heading where status='active' and heading='$general_head' and websitetype='hindi' order by position");
            	if( mysql_num_rows( $query_sub ) > 0 ) { ?>
				<!-- .sidebar -->
				<div class="col-lg-3 col-md-3 col-sm-12 col-xs-12">
                <div><img src="img/ff.jpg" alt=""></div>
                    ...
                    <h2 class="cat-title" style="font-size: 20px;">
                    <?php	
                    $mainHeadName = mysql_fetch_array( mysql_query( "select name from heading where sno = '$general_head' and websitetype='hindi'"));
                    echo $mainHeadName['name'];
					?></h2>
                        <ul class="vf-megamenu clearfix megamenu-content">
                         <?php
