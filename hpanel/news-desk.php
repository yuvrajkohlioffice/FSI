...
if(isset($_GET['action']) && !empty($_GET['action']) && $_GET['action']=='active')
{
	...
	if($capture==$postcapture && $capture!="" && $postcapture!="")
	{
		...
		$query="update news set status='inactive' where ser='$unid_id'";
		...
		if($rs)
		{
			echo "<script>location.href='news-desk.php?display=status'</script>";	
		}
		else
		{
			echo "<script>location.href='news-desk.php?error=incorrect'</script>";	
		}
	}
	else
	{
		echo "<p align='center'>Please Wait..</p>";
		echo "<script>location.href='news-desk.php?status=codeinvalid'</script>";
	}
}
...
if(isset($_GET['action']) && !empty($_GET['action']) && $_GET['action']=='inactive')
{
	...
	if($capture==$postcapture && $capture!="" && $postcapture!="")
	{
		...
		$query="update news set status='active' where ser='$unid_id'";
		$rs=mysql_query($query);
		$md5_hash = md5(rand(0,999)); 
		$security_code = substr($md5_hash, 16, 16); 
		$_SESSION['capture_code']=$security_code;
		if($rs)
		{
			echo "<script>location.href='news-desk.php?display=update'</script>";	
		}
		else
		{
			echo "<script>location.href='news-desk.php?error=incorrect'</script>";	
...
if(isset($_POST['flag']) && !empty($_POST['flag']) && $_POST['flag']==1)
{
	...
	if($capture==$postcapture && $capture!="" && $postcapture!="")
	{
		...
		$img_response = sh_upload_file("txtbanner", "image", "../uploads/news/", "img_".$ran);
		if($img_response)
		{
			$fname = $img_response;
		}
		...
		$doc_response = sh_upload_file2("txtfile", "file", "../uploads/news/", "news_".$ran);
		...
		$query="insert into news set newsdate='$news_date', areaSection = '$_POST[areaSection]', title='".filter_xss($_POST['txtheading'])."',description='$desc',friendly_url='".filter_xss($_POST['friendly_url'])."', link='".filter_xss($_POST['url'])."', target='$_POST[target]',status='active', extension='$iname',image='$fname', bold = '', size = '', imageicon = '', archievecategory = '', lastmodify = '$crdate', ntype = '', year = '".date("Y", strtotime($news_date))."', websitetype = 'english', crdate='$crdate',sorder='".filter_xss($_POST['txtposition'])."'";
...
if(isset($_GET['action']) && !empty($_GET['action']) && $_GET['action']=='delete_heading')
{
	...
	if($capture==$postcapture && $capture!="" && $postcapture!="")
	{
		...
		$query="delete from news where ser='$unid_id'";
		$rs=mysql_query($query);
		$md5_hash = md5(rand(0,999)); 
		$security_code = substr($md5_hash, 16, 16); 
		$_SESSION['capture_code']=$security_code;
		if($rs)
		{
			echo "<script>location.href='news-desk.php?display=delete'</script>";
		}
		else
		{
			echo "<script>location.href='news-desk.php?error=incorrect'</script>";
...
if(isset($_POST['action']) && !empty($_POST['action']) && $_POST['action']=='edit1')
{
	...
	if($capture==$postcapture && $capture!="" && $postcapture!="")
	{
		...
		$doc_response = sh_upload_file("txtfile", "file", "../uploads/news/", "news_".$ran);
		if($doc_response)
		{
			$iname = $doc_response;
		}
		...
		$query="update news set newsdate='$news_date', areaSection = '$_POST[areaSection]', title='".filter_xss($_POST[txtheading])."',description='$desc',friendly_url='".filter_xss($_POST[friendly_url])."', link='".filter_xss($_POST[url])."', target='$_POST[target]',sorder='".filter_xss($_POST[txtposition])."', bold = '', size = '', imageicon = '', archievecategory = '', ntype = '', year = '".date("Y", strtotime($news_date))."', websitetype = 'english'";
		...
		$query.=", lastmodify='$crdate' where ser='$unid_id'";
		...
		if($rs)
		{
			...
			$url = "news-desk.php?display=update"; 
...
function view()
{
	...
	*/
	...
	$targetpage = "news-desk.php"; 	//your file name  (the name of this file)
	$limit = 10; 	
	$page = 0;
	if(isset($_GET['page']))	
	...
	$sql = "select * from news order by ser desc LIMIT $start, $limit";
			...
			<div class="container-fluid">
				...
				<div class="row-fluid">
					<div class="span12">						     
						<h3 class="page-title">
							News Management
							<small>Add/Edit Website News / Events</small>
						</h3>
						<ul class="breadcrumb">
							<li>
								<i class="icon-home"></i>
								...
								<table class="table table-hover">
									...
									?>
										<tr>
											...
											<td>
											<?php
												echo htmlspecialchars($row_content['areaSection'], ENT_QUOTES);
											?>
											</td>
											<td>
											<?php
											...
											<td>
											<a href="news-desk.php?action=<?php echo $row_content['status']?>&sno=<?php echo $row_content['ser']?>&log=<?php echo $_SESSION['capture_code'];?>">
											<?php
											...
											<td>
											<a href="news-desk.php?sno=<?php echo $row_content['ser'];?>&action=<?php echo 'edit_heading'?>&log=<?php echo $_SESSION['capture_code'];?>">
											<img border="0" src="images/edit.png" width="20" height="20" alt="Edit" name="Edit"></a></td>
											<td><input type="image" img border="0" src="images/delete.png" width="20" height="20" onclick="var a=confirm('Are You sure delete this record from database ?');if(a==true){location.href='news-desk.php?sno=<?php echo $row_content['ser']?>&action=<?php echo 'delete_heading'?>&log=<?php echo $_SESSION['capture_code'];?>';}" alt="Delete" name="delete"></td>
											<td>
											<?php
					...
					<div class="span12">
						...
						<div class="portlet box purple">
							...
							<div class="portlet-body form">
								<!-- BEGIN FORM-->
								<form action="news-desk.php?action=category" enctype="multipart/form-data" id="form_page" name="form_page" class="form-horizontal" method="post">
									<div class="control-group">
										<label class="control-label">Area<span class="required">*</span></label>
										<div class="controls">
											<select class="span6 m-wrap" name="areaSection">
												<option value="">Select...</option>
												<option value="news">News Section</option>
												<option value="marquee">Marquee Section</option>
									...
									<div class="control-group">
										<label class="control-label">News Heading<span class="required">*</span></label>
										<div class="controls">
...
function edit()
{
	...
	{	
		...
		$query=mysql_query("select * from news where ser='$contentid'");
			...
			<div class="container-fluid">
				...
				<div class="row-fluid">
					...
					<div class="span12">
						...
						<div class="portlet box purple">
							<div class="portlet-title">
								<div class="caption"><i class="icon-reorder"></i>Edit Page Content - <b>"<?php echo filter_xss($row_content['title']);?>"</b></div>
								<div class="tools">
									<a href="javascript:;" class="collapse"></a>
									<!--a href="#portlet-config" data-toggle="modal" class="config"></a-->
									<a href="javascript:;" class="reload"></a>
							...
							<div class="portlet-body form">
								<!-- BEGIN FORM-->
								<form action="news-desk.php?action=category" enctype="multipart/form-data" id="form_page" name="editform_page" class="form-horizontal" method="post">
									<div class="control-group">
										<label class="control-label">Area<span class="required">*</span></label>
										<div class="controls">
											<select class="span6 m-wrap" name="areaSection">
												<option value="">Select...</option>
												<option value="news" <?php if($row_content['areaSection']=='news'){ echo 'selected';}?>>News Section</option>
												<option value="marquee" <?php if($row_content['areaSection']=='marquee'){ echo 'selected';}?>>Marquee Section</option>
												<option value="both" <?php if($row_content['areaSection']=='both'){ echo 'selected';}?>>Both</option>
											</select>
										</div>
									</div>
									<div class="control-group">
										<label class="control-label">News Date</label>
										<div class="controls">
											<input class="span6 m-wrap" size="16" type="text" name="newsdate" autocomplete="off" value="<?php echo date('d-M-Y',strtotime($row_content['newsdate']));?>" id="ui_date_picker_change_year_month"/>
										</div>
									</div>
									<div class="control-group">
										<label class="control-label">News Heading<span class="required">*</span></label>
										<div class="controls">
											<input type="text" name="txtheading" value="<?php echo filter_xss($row_content['title']);?>" data-required="1" class="span6 m-wrap"/>
										</div>
									</div>
									<div class="control-group">
										<label class="control-label">Menu Slug / Friendly URL&nbsp;&nbsp;</label>
									...
									<div class="control-group">
										...
										<div class="controls">
											...
											<?php
											if($row_content['image']!='' && file_exists("../uploads/news/".$row_content['image']))
											{
											?>
											<a href="../uploads/news/<?php echo $row_content['image']?>" title="View Attachment" target="_blank">
									...
									<div class="control-group">
										...
										<div class="controls">
											...
											<?php
													if($row_content['extension']!='' && file_exists("../uploads/news/".$row_content['extension']))
													{
													?>
													<a href="../uploads/news/<?php echo $row_content['extension']?>" title="View Attachment" target="_blank">
													<font color="#6A9D00" size="2" face="Trebuchet MS">view attachment</font></a>
													<?php
													}
													?>
