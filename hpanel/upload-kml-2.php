...
if(isset($_POST['flag']) && !empty($_POST['flag']) && $_POST['flag']==1)
{
	...
	if($capture==$postcapture && $capture!="" && $postcapture!="")
	{
		...
		$index=0;
		$state=mysql_query("select name,sno from state_master where status='active' order by name limit 18,20");
		...
		if($rs)
		{
			echo "<script>location.href='upload-kml-2.php?display=add'</script>";
		}
		else
		{
			echo "<script>location.href='upload-kml-2.php?error=incorrect'</script>";
		}
	}
	else
	{
		echo "<p align='center'>Please Wait..</p>";
		echo "<script>location.href='upload-kml-2.php?status=codeinvalid'</script>";
	}
}
...
if(isset($_GET['action']) && !empty($_GET['action']) && $_GET['action']=='delete_heading')
{
	...
	if($capture==$postcapture && $capture!="" && $postcapture!="")
	{
		...
		if($rs)
		{
			echo "<script>location.href='upload-kml-2.php?display=delete'</script>";
		}
		else
		{
			echo "<script>location.href='upload-kml-2.php?error=incorrect'</script>";
...
if(isset($_POST['action']) && !empty($_POST['action']) && $_POST['action']=='edit1')
{
	...
	if($capture==$postcapture && $capture!="" && $postcapture!="")
	{
		...
		if($rs)
		{
			echo "<script>location.href='upload-kml-2.php?display=update'</script>";
		}
		else
		{
			echo "<script>location.href='upload-kml-2.php?error=incorrect'</script>";
...
if(isset($_GET['action']) && !empty($_GET['action']) && $_GET['action']=='off')
{
	...
	if($capture==$postcapture && $capture!="" && $postcapture!="")
	{
		echo $sql="update upload_kml set status='inactive' where ser='$unid_id'";
		$rs = mysql_query($sql);
...
function view()
{
	...
	*/
	...
	$targetpage = "upload-kml-2.php"; 	//your file name  (the name of this file)
				...
				<div class="row-fluid">
					<div class="span12">						     
						...
						<ul class="breadcrumb">
							...
							<li><a href="#">Upload KML Files (L-Z)</a></li>
						</ul>
					</div>
				</div>
				<!-- END PAGE HEADER-->
								...
								<table class="table table-hover">
									...
									?>
										<tr>
											...
											<td>
											<?php
											if($row_content['extension']!="" && file_exists("../uploads/kmlfiles/$row_content[extension]"))
											{
											...
											{
											?>
											<a href="upload-kml-2.php?action=on&ser=<?php echo $row_content['ser'];?>&log=<?php echo $_SESSION['capture_code'];?>"><img src=images/off.gif border=0 title='Inactive' width=16 height=16></a>
											<?php											
											}
											else
											{
											?>
											<a href="upload-kml-2.php?action=off&ser=<?php echo $row_content['ser'];?>&log=<?php echo $_SESSION['capture_code'];?>"><img src=images/on.gif border=0 title='Active' width=16 height=16></a>
											<?php
											}
											?>											</td>
											<td>
											<a href="upload-kml-2.php?sno=<?php echo $row_content['ser'];?>&action=<?php echo 'edit_heading'?>&log=<?php echo $_SESSION['capture_code'];?>">
											<img border="0" src="images/edit.png" width="20" height="20" alt="Edit" name="Edit"></a></td>
											<td><input type="image" img border="0" src="images/delete.png" width="20" height="20" onclick="var a=confirm('Are You sure delete this record from database ?');if(a==true){location.href='upload-kml-2.php?sno=<?php echo $row_content['ser']?>&action=<?php echo 'delete_heading'?>&log=<?php echo $_SESSION['capture_code'];?>';}" alt="Delete" name="delete"></td>
					...
					<div class="span12">
						...
						<div class="portlet box purple">
							...
							<div class="portlet-body form">
								<!-- BEGIN FORM-->
								<form action="upload-kml-2.php?action=category" enctype="multipart/form-data" id="form_page" name="form_page" class="form-horizontal" method="post">
				...
				<div class="row-fluid">
					...
					<div class="span12">
						...
						<div class="portlet box purple">
							...
							<div class="portlet-body form">
								<!-- BEGIN FORM-->
								<form action="upload-kml-2.php?action=category" enctype="multipart/form-data" id="form_page" name="editform_page" class="form-horizontal" method="post">
...
<?php
	...
	{
		echo "<script>location.href='upload-kml-2.php';</script>";
	}
}
?>