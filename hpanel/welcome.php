...
if(isset($_GET['action']) && !empty($_GET['action']) && $_GET['action']=='delete_heading')
{
	...
	if($capture==$postcapture && $capture!="" && $postcapture!="")
	{
		...
		$query="delete from welcome where sno='$unid_id'";
		...
		if($rs)
		{
			echo "<script>location.href='welcome.php?display=delete'</script>";
		}
		else
		{
			echo "<script>location.href='welcome.php?error=incorrect'</script>";
		}
	}
	else
	{
		echo "<p align='center'>Please Wait..</p>";
		echo "<script>location.href='welcome.php?status=codeinvalid'</script>";
	}
}
...
if(isset($_POST['action']) && !empty($_POST['action']) && $_POST['action']=='edit1')
{
	...
	if($capture==$postcapture && $capture!="" && $postcapture!="")
	{
		$sql_off="select * from welcome where websitetype='english'";
		...
		if($row==0)
		{
			...
			if($img_response)
			{
				$query="insert into welcome(websitetype,front,detail,friendly_url, otherurl,target,crdate,ipaddress,userid,heading,banner) values('$_POST[language]','$desc','$desc1','".filter_xss($_POST['friendlyurl'])."','".filter_xss($_POST['url'])."','$_POST[target]','$pdate','$_SESSION[REMOTE_ADDR]','$_SESSION[uname]','".filter_xss($_POST['txtheading'])."','$fname')";
			}
			else
			{
				$query="insert into welcome(websitetype,front,detail,friendly_url, otherurl,target,crdate,ipaddress,userid,heading) values('$_POST[language]','$desc','$desc1','".filter_xss($_POST['friendlyurl'])."','".filter_xss($_POST['url'])."','$_POST[target]','$pdate','$_SESSION[REMOTE_ADDR]','$_SESSION[uname]','".filter_xss($_POST['txtheading'])."')";
			}
		...
		else
		{
			if($img_response)
			{
				$query="update welcome set websitetype='$_POST[language]',front='$desc',detail='$desc1',friendly_url='".filter_xss($_POST['friendlyurl'])."',otherurl='".filter_xss($_POST['url'])."',target='$_POST[target]',modifyby='$_SESSION[uname]',heading='".filter_xss($_POST['txtheading'])."',modifydate='$pdate',ipaddress='$_SERVER[REMOTE_ADDR]',banner='$fname' ";
			}
			else
			{
				$query="update welcome set websitetype='$_POST[language]',front='$desc',detail='$desc1',friendly_url='".filter_xss($_POST['friendlyurl'])."',otherurl='".filter_xss($_POST['url'])."',target='$_POST[target]',modifyby='$_SESSION[uname]',heading='".filter_xss($_POST['txtheading'])."',modifydate='$pdate',ipaddress='$_SERVER[REMOTE_ADDR]' ";
			}
		}
		...
		if($rs)
		{
			echo "<script>location.href='welcome.php?display=update'</script>";	
		}
		else
		{
			echo "<script>location.href='welcome.php?error=incorrect'</script>";	
...
function view()
{
	$res_details=mysql_query("select * from welcome");
					...
					<div class="span12">
						...
						<div class="portlet box purple">
							...
							<div class="portlet-body form">
								<!-- BEGIN FORM-->
								<form action="welcome.php" enctype="multipart/form-data" id="form_page" name="form_page" class="form-horizontal" method="post">
									...
									<div class="control-group">
										...
										<div class="controls">
											<select class="span6 m-wrap" name="language">
												...
												<option value="english" <?php if($row_details['websitetype']=='english'){ echo 'selected';}?>>English</option>
											</select>											
										</div>
									</div>
									...
									<div class="control-group">
										...
										<div class="controls">
											...
											?></font></b></font>
																						...
																						<label onClick="var a=confirm('Are You sure delete this from database ?');if(a==true){location.href='welcome.php?action=remove_banner&ID=about';}"><font size="2" face="Trebuchet MS" color="#FF0000">
