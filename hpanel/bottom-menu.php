...
if(isset($_GET['action']) && !empty($_GET['action']) && $_GET['action']=='active')
{
	...
	{	
		...
		$query="update bottom_menu set status='inactive' where sno='$unid_id'";
		...
		if($rs)
		{
			echo "<script>location.href='bottom-menu.php?display=update'</script>";	
		}
		else
		{
			echo "<script>location.href='bottom-menu.php?error=incorrect'</script>";	
		}
	}
	else
	{
		echo "<p align='center'>Please Wait..</p>";
		echo "<script>location.href='bottom-menu.php?status=codeinvalid'</script>";
	}
}
...
if(isset($_GET['action']) && !empty($_GET['action']) && $_GET['action']=='inactive')
{
	...
	{	
		...
		$query="update bottom_menu set status='active' where sno='$unid_id'";
		$rs=mysql_query($query);
...
if(isset($_POST['flag']) && !empty($_POST['flag']) && $_POST['flag']==1)
{
	...
	{	
		...
		$query="insert into bottom_menu(websitetype, name,status,position,otherurl,target) values('$_POST[website_type]','$title','active','$position','".$url."','$_POST[target]')";
		...
		if($rs)
		{
			echo "<script>location.href='bottom-menu.php?display=add'</script>";
		}
		else
		{
			echo "<script>location.href='bottom-menu.php?error=incorrect'</script>";
...
if(isset($_GET['action']) && !empty($_GET['action']) && $_GET['action']=='delete_heading')
{
	...
	if($capture==$postcapture && $capture!="" && $postcapture!="")
	{		
		$query="delete from bottom_menu where sno='$unid_id'";
		$rs=mysql_query($query);
		$md5_hash = md5(rand(0,999)); 
		$security_code = substr($md5_hash, 16, 16); 
		$_SESSION['capture_code']=$security_code;
		if($rs)
		{
			echo "<script>location.href='bottom-menu.php?display=delete'</script>";
		}
		else
		{
			echo "<script>location.href='bottom-menu.php?error=incorrect'</script>";
...
if(isset($_POST['action']) && !empty($_POST['action']) && $_POST['action']=='edit1')
{
	...
	{	
		...
		$query="update bottom_menu set websitetype='$_POST[website_type]',name='$title',position='$position',otherurl='".$url."',target='$_POST[target]' where sno='$unid_id'";
			...
			<div class="container-fluid">
				...
				<div class="row-fluid">
					<div class="span12">						     
						<h3 class="page-title">
							Website Bottom Menu
							<small>Add Website Bottom Menu</small>
						</h3>
						<ul class="breadcrumb">
							<li>
								<i class="icon-home"></i>
							...
							<li><a href="#">Manage Bottom Menu's</a></li>
						</ul>
					</div>
				</div>
				<!-- END PAGE HEADER-->
					...
					<div class="span12">
						...
						<div class="portlet box blue">
							<div class="portlet-title">
								<div class="caption"><i class="icon-cogs"></i>Bottom Menu's</div>
								<div class="tools">
							...
							<div class="portlet-body">
								<table class="table table-hover">
									...
									<?php
									$sno = 0;
									$res_mainmenu=mysql_query("select * from bottom_menu order by position asc");
									...
									?>
										<tr>
											...
											<td>
											<a href="bottom-menu.php?action=<?php echo $row_mainmenu['status']?>&sno=<?php echo $row_mainmenu['sno']?>&log=<?php echo $_SESSION['capture_code'];?>">
											<?php
											...
											<td>
											<a href="bottom-menu.php?sno=<?php echo $row_mainmenu['sno'];?>&action=<?php echo 'edit_heading'?>">
											<img border="0" src="images/edit.png" width="20" height="20" alt="Edit" name="Edit"></a></td>
											<td><input type="image" img border="0" src="images/delete.png" width="20" height="20" onclick="var a=confirm('Are You sure delete this record from database ?');if(a==true){location.href='bottom-menu.php?sno=<?php echo $row_mainmenu['sno']?>&action=<?php echo 'delete_heading'?>&log=<?php echo $_SESSION['capture_code'];?>';}" alt="Delete" name="delete"></td>
										</tr>
										<?php
										}
										?>
					...
					<div class="span12">
						...
						<div class="portlet box purple">
							...
							<div class="portlet-body form">
								<!-- BEGIN FORM-->
								<form action="bottom-menu.php?action=category" id="form_main_menu" class="form-horizontal" method="post">
...
function edit()
{
	...
	{	
		...
		$query=mysql_query("select * from bottom_menu where sno='$menuid'");
			...
			<div class="container-fluid">
				...
				<div class="row-fluid">
					<div class="span12">
						...
						<ul class="breadcrumb">
							...
							<li>
								<a href="bottom-menu.php">Menu Master's</a>
								<span class="icon-angle-right"></span>
							</li>
							<li><a href="#">Edit Bottom Menu's</a></li>
						</ul>
					</div>
				</div>
				<!-- END PAGE HEADER-->
				...
				<div class="row-fluid">
					...
					<div class="span12">
						...
						<div class="portlet box purple">
							<div class="portlet-title">
								<div class="caption"><i class="icon-reorder"></i>Edit Bottom Menu - <b>"<?php echo $row_mainmenu['name'];?>"</b></div>
								<div class="tools">
									<a href="javascript:;" class="collapse"></a>
									<!--a href="#portlet-config" data-toggle="modal" class="config"></a-->
									<a href="javascript:;" class="reload"></a>
							...
							<div class="portlet-body form">
								<!-- BEGIN FORM-->
								<form action="bottom-menu.php" id="form_main_menu" class="form-horizontal" method="post">									
									<div class="control-group">
										<label class="control-label">Display in Which Website?<span class="required">*</span></label>
										<div class="controls">
											<select class="span6 m-wrap" name="website_type">
...
<?php
	...
	{
		echo "<script>location.href='bottom-menu.php';</script>";
	}
}
?>
