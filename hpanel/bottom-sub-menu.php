...
{
	$query="update bottom_sub_heading set status='inactive' where sno='$_GET[sno]'";
	$rs=mysql_query($query);
	
	if($rs)
	{
		echo "<script>location.href='bottom-sub-menu.php?display=update'</script>";	
	}
	else
	{
		echo "<script>location.href='bottom-sub-menu.php?error=incorrect'</script>";	
	}
}
...
if($_GET['action']=='inactive')
{
	...
	$query="update bottom_sub_heading set status='active' where sno='$_GET[sno]'";
	$rs=mysql_query($query);
...
if($_GET['action']=='yes')
{
	$query="update bottom_sub_heading set home='no' where sno='$_GET[sno]'";
	$rs=mysql_query($query);
...
if($_GET['action']=='no')
{
	$query="update bottom_sub_heading set home='yes' where sno='$_GET[sno]'";
	$rs=mysql_query($query);
...
if($_POST['flag']==1)
{
	...
	$query="insert into bottom_sub_heading(heading, subhead,status,position,otherurl,target) values('$_POST[parent]','$_POST[txtheading]','active','$_POST[txtposition]','$_POST[url]','$_POST[target]')";

	$rs=mysql_query($query);
	if($rs)
	{
		echo "<script>location.href='bottom-sub-menu.php?display=add'</script>";
	}
	else
	{
		echo "<script>location.href='bottom-sub-menu.php?error=incorrect'</script>";
...
{
	$query="delete from bottom_sub_heading where sno='$_GET[sno]'";
	$rs=mysql_query($query);
	
	if($rs)
	{
		echo "<script>location.href='bottom-sub-menu.php?display=delete'</script>";
	}
	else
	{
		echo "<script>location.href='bottom-sub-menu.php?error=incorrect'</script>";
...
if($_POST['action']=='edit1')
{
	...
	$query="update bottom_sub_heading set heading='$_POST[parent]', subhead='$_POST[txtheading]',otherurl='$_POST[url]',target='$_POST[target]', position='$_POST[txtposition]' where sno='$_POST[sno]'";
						...
						<div class="portlet box blue">
							...
							<div class="portlet-body">
								<table class="table table-hover">
									...
									<tbody>
									<?php
									$res_mainmenu=mysql_query("select name,sno from bottom_menu order by position asc");
									...
									?>										
										<tr>
											...
											<?php
												$count=0;
												$res_submenu=mysql_query("select sno,heading,subhead,otherurl,target,status from bottom_sub_heading where heading='$row_mainmenu[sno]' order by position asc");
											...
											?>
												<tr>
													...
													<td width="36">
													<a href="bottom-sub-menu.php?action=<?php echo $row_submenu['status']?>&sno=<?php echo $row_submenu['sno']?>">
													<?php
													...
													<td width="19">
													<a href="bottom-sub-menu.php?sno=<?php echo $row_submenu['sno'];?>&action=<?php echo 'edit_heading'?>">
													<img border="0" src="images/edit.png" width="20" height="20" alt="Edit" name="Edit"></a></td>
													<td width="18"><input type="image" img border="0" src="images/delete.png" width="20" height="20" onclick="var a=confirm('Are You sure delete this record from database ?');if(a==true){location.href='bottom-sub-menu.php?sno=<?php echo $row_submenu[sno]?>&action=<?php echo 'delete_heading'?>';}" alt="Delete" name="delete"></td>
												</tr>
											<?php
												}
											?>
					...
					<div class="span12">
						...
						<div class="portlet box purple">
							...
							<div class="portlet-body form">
								<!-- BEGIN FORM-->
								<form action="bottom-sub-menu.php?action=category" id="form_main_menu" class="form-horizontal" method="post">
									...
									<div class="control-group">
										...
										<div class="controls">
											<select class="span6 m-wrap" name="parent" size="1">
												...
												<?php
													$res_main=mysql_query("select name,sno from bottom_menu where status='active'");
...
function edit()
{
		...
		$query=mysql_query("select * from bottom_sub_heading where sno='$menuid'") or die(mysql_error());
				...
				<div class="row-fluid">
					<div class="span12">
						...
						<ul class="breadcrumb">
							...
							<li>
								<a href="bottom-sub-menu.php">Menu Master's</a>
								<span class="icon-angle-right"></span>
							</li>
				...
				<div class="row-fluid">
					...
					<div class="span12">
						...
						<div class="portlet box purple">
							...
							<div class="portlet-body form">
								<!-- BEGIN FORM-->
								<form action="bottom-sub-menu.php" id="form_main_menu" class="form-horizontal" method="post">									
									<div class="control-group">
