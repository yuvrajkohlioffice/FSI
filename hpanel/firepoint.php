...
if(isset($_GET['status']) && !empty($_GET['status']) && $_GET['status']=='upload')
{
	upload_csv();
	exit;
}
...
if(isset($_POST['action']) && !empty($_POST['action']) && $_POST['action']=='upload')
{
	...
	if($capture==$postcapture && $capture!="" && $postcapture!="")
	{
		...
		//if($_FILES['upload']['name']!='' && $type=='application/vnd.ms-excel' && $original_extension == ".csv" && $size <= $default_size)
		{
			...
			$uploadedpath = "../uploads/firepoints/";
			$dest = $uploadedpath.'firepoints'.$ext;
		
			if( move_uploaded_file($source,$dest) )
			{
				rename($dest,$dest);
				echo "<script>alert('File successfully uploaded');location.href='fire-point.php?status=upload';</script>";
			}
		}
		/*else
		{			
			echo "<script>alert('Please Upload CSV file only');location.href='firepoint.php';</script>";
		}*/
	}
	else
	{
	...
	<table border="0" cellpadding="5" cellspacing="3" style="border:1px solid #800000; border-collapse: collapse" width="100%" bordercolor="#000000" id="table5">
		<tr>
			...
			<div align="center">
				...
				<p><b><font face="Verdana" size="2">Please do
				<font color="#800000">'Firedate'</font> in <font color="#800000">
				mm/dd/yyyy</font> format. then upload the <font color="#800000">
				csv</font> file</font></b><br>
&nbsp;</div>
			</td>
		</tr>
		<tr>
			<td>
			<form method="POST" enctype="multipart/form-data" action="firepoint.php">
...
<body onload="burstCache();" class="page-header-fixed">
	...
	<div class="page-container row-fluid">
					...
					<div class="span12">
						...
						<div class="portlet box purple">
							...
							<div class="portlet-body form">
								...
								<form name="f1" method="post" action="firepoint.php">
								<table border="0" cellpadding="2" style="border-collapse: collapse" width="100%" bordercolor="#800000" id="table1">
									...
									<tr>
										<td>
				<table border="0" cellpadding="3" style="border-collapse: collapse" width="100%" bordercolor="#000000" id="table7" cellspacing="1">
					<tr>
						<td align="center" bgcolor="#E9E9E9">
						<font style="font-size: 10pt; font-weight: 700" face="Verdana">
						Fire Date</font></td>
		...
		?>
					<tr>
						<td align="center">
						<?php echo addslashes($data[0]); ?></td>
						<td align="center">
						<?php echo addslashes($data[1]); ?></td>
						<td align="center">
						<?php echo addslashes($data[2]); ?></td>
						<td align="center">
						<?php echo addslashes($data[3]); ?></td>
						<td align="center">
						<?php echo addslashes($data[4]); ?></td>
						<td align="center">
						<?php echo addslashes($data[5]); ?></td>
						<td align="center">
						<?php echo addslashes($data[6]); ?></td>
					</tr><?php } ?>
					</table>
										<p align="center">
