...
if(isset($_POST['flag']) && !empty($_POST['flag']) && $_POST['flag'])
{
	...
	if($capture==$postcapture && $capture!="" && $postcapture!="")
	{
		...
		$doc_response = sh_upload_file2("txtfile", "file", "../uploads/documents/", "doc_".$ran);
		if($doc_response)
		{
			$fname = $doc_response;
			$query="insert into uploads set filename='$fname',crdate='$crdate',ip_address='$_SERVER[REMOTE_ADDR]'";
		...
		if($rs)
		{
			echo "<script>location.href='upload-files1.php?display=add'</script>";
		}
		else
		{
			echo "<script>location.href='upload-files1.php?error=incorrect'</script>";
		}
	}
	else
	{
		echo "<p align='center'>Please Wait..</p>";
		echo "<script>location.href='upload-files1.php?status=codeinvalid'</script>";
	}
}
...
if(isset($_GET['action']) && !empty($_GET['action']) && $_GET['action']=='delete_heading')
{
	...
	if($capture==$postcapture && $capture!="" && $postcapture!="")
	{
		...
		if($rs)
		{
			echo "<script>location.href='upload-files1.php?display=delete'</script>";
		}
		else
		{
			echo "<script>location.href='upload-files1.php?error=incorrect'</script>";
...
if(isset($_POST['action']) && !empty($_POST['action']) && $_POST['action']=='edit1')
{
	...
	if($capture==$postcapture && $capture!="" && $postcapture!="")
	{
		...
		if($rs)
		{
			echo "<script>location.href='upload-files1.php?display=update'</script>";
		}
		else
		{
			echo "<script>location.href='upload-files1.php?error=incorrect'</script>";
...
function view()
{
	...
	*/
	...
	$targetpage = "upload-files1.php"; 	//your file name  (the name of this file)
	$limit = 10; 
								...
								<table class="table table-hover">
									...
									?>
										<tr>
											...
											<td>
											<a href="upload-files1.php?sno=<?php echo $row_content['sno'];?>&action=<?php echo 'edit_heading'?>&log=<?php echo $_SESSION['capture_code'];?>">
											<img border="0" src="images/edit.png" width="20" height="20" alt="Edit" name="Edit"></a></td>
											<td><input type="image" img border="0" src="images/delete.png" width="20" height="20" onclick="var a=confirm('Are You sure delete this record from database ?');if(a==true){location.href='upload-files1.php?sno=<?php echo $row_content['sno']?>&action=<?php echo 'delete_heading'?>&log=<?php echo $_SESSION['capture_code'];?>';}" alt="Delete" name="delete"></td>
					...
					<div class="span12">
						...
						<div class="portlet box purple">
							...
							<div class="portlet-body form">
								<!-- BEGIN FORM-->
								<form action="upload-files1.php?action=category" enctype="multipart/form-data" id="form_page" name="form_page" class="form-horizontal" method="post">
				...
				<div class="row-fluid">
					...
					<div class="span12">
						...
						<div class="portlet box purple">
							...
							<div class="portlet-body form">
								<!-- BEGIN FORM-->
								<form action="upload-files1.php?action=category" enctype="multipart/form-data" id="form_page" name="editform_page" class="form-horizontal" method="post">
...
<?php
	...
	{
		echo "<script>location.href='upload-files1.php';</script>";
	}
}
?>