...
if(isset($_GET['action']) && !empty($_GET['action']) && $_GET['action']=='active')
{
	...
	{
		$query="update tenders set status='inactive' where ser='$unid_id'";
	...
	$_SESSION['capture_code']=$security_code;
		if($rs)
		{
			echo "<script>location.href='tenders.php?display=status'</script>";	
		}
		else
		{
			echo "<script>location.href='tenders.php?error=incorrect'</script>";	
		}
	}
	else
	{
		echo "<p align='center'>Please Wait..</p>";
		echo "<script>location.href='tenders.php?status=codeinvalid'</script>";
	}
}
...
if(isset($_GET['action']) && !empty($_GET['action']) && $_GET['action']=='inactive')
{
	...
	if($capture==$postcapture && $capture!="" && $postcapture!="")
	{	
		$query="update tenders set status='active' where ser='$unid_id'";
		$rs=mysql_query($query);
		$md5_hash = md5(rand(0,999)); 
	$security_code = substr($md5_hash, 15, 6); 
	$_SESSION['capture_code']=$security_code;
		if($rs)
		{
			echo "<script>location.href='tenders.php?display=update'</script>";	
		}
		else
		{
			echo "<script>location.href='tenders.php?error=incorrect'</script>";	
...
if(isset($_POST['flag']) && !empty($_POST['flag']) && $_POST['flag']==1)
{
	...
	if($capture==$postcapture && $capture!="" && $postcapture!="")
	{
		...
		$doc_response = sh_upload_file("txtfile", "file", "../uploads/tenders/", "ten_".$ran);
		if($doc_response)
		{
			$fname = $doc_response;
		}
		...
		$op_date=date("Y-m-d",strtotime($_POST['opdate']));
		
		$insert="insert into tenders(startdate,lastdate,openingdate, reference_no,title,description,link,status,extension,image,crdate,lastmodify,target,sorder) values('$start_date','$last_date','$op_date','".filter_xss($_POST[ref_no])."','".filter_xss($_POST[txttitle])."','".filter_xss($_POST[description])."','".filter_xss($_POST[txtlink])."','active','$fname','$iname','$crdate','$crdate','$_POST[target]','".filter_xss($_POST[sorder])."')";
...
if(isset($_GET['action']) && !empty($_GET['action']) && $_GET['action']=='delete_heading')
{
	...
	if($capture==$postcapture && $capture!="" && $postcapture!="")
	{
		$query="delete from tenders where ser='$unid_id'";
		$rs=mysql_query($query);
			$md5_hash = md5(rand(0,999)); 
	$security_code = substr($md5_hash, 15, 6); 
	$_SESSION['capture_code']=$security_code;
		if($rs)
		{
			echo "<script>location.href='tenders.php?display=delete'</script>";
		}
		else
		{
			echo "<script>location.href='tenders.php?error=incorrect'</script>";
...
if(isset($_GET['action']) && !empty($_GET['action']) && $_POST['action']=='edit1')
{
	$capture=preg_replace('/[^a-zA-Z0-9]/', '', trim($_SESSION['capture_code']));
	$postcapture=preg_replace('/[^a-zA-Z0-9]/', '', trim($_POST['log']));
	...
	if($capture==$postcapture && $capture!="" && $postcapture!="")
	{
		...
		$update="update tenders set startdate='$start_date',lastdate='$last_date',openingdate='$op_date', reference_no='".filter_xss($_POST[ref_no])."',title='".filter_xss($_POST[title])."',description='".filter_xss($_POST[description])."',link='".filter_xss($_POST[link])."',lastmodify='$crdate',target='$_POST[target]',sorder='".filter_xss($_POST[sorder])."'";
	...
	$_SESSION['capture_code']=$security_code;
		if($rs)
		{
			$url = "tenders.php?display=update"; 
...
function view()
{
	...
	$tbl_name="tenders";		//your table name
	...
	*/
	...
	$targetpage = "tenders.php"; 	//your file name  (the name of this file)
	$limit = 10; 	
	$page = 0;
	if(isset($_GET['page']))	
	...
	$sql = "select * from tenders order by ser desc LIMIT $start, $limit";
								...
								<table class="table table-hover">
									...
									?>
										<tr>
											...
											<td>
											<a href="tenders.php?action=<?php echo $row_content['status']?>&sno=<?php echo $row_content['ser']?>&log=<?php echo $_SESSION['capture_code'];?>">
											<?php
											...
											<td>
											<a href="tenders.php?sno=<?php echo $row_content['ser'];?>&action=<?php echo 'edit_heading'?>&log=<?php echo $_SESSION['capture_code'];?>">
											<img border="0" src="images/edit.png" width="20" height="20" alt="Edit" name="Edit"></a></td>
											<td><input type="image" img border="0" src="images/delete.png" width="20" height="20" onclick="var a=confirm('Are You sure delete this record from database ?');if(a==true){location.href='tenders.php?sno=<?php echo $row_content['ser']?>&action=<?php echo 'delete_heading'?>&log=<?php echo $_SESSION['capture_code'];?>';}" alt="Delete" name="delete"></td>
											<td>
											<?php
					...
					<div class="span12">
						...
						<div class="portlet box purple">
							...
							<div class="portlet-body form">
								<!-- BEGIN FORM-->
								<form action="tenders.php?action=category" enctype="multipart/form-data" id="form_page" name="form_page" class="form-horizontal" method="post">
									...
									<div class="control-group">
										...
										<div class="controls">
											<input class="m-wrap" size="16" type="date" name="opdate" autocomplete="off" value="" id="ui_date_picker_change_year_month1"/>
										</div>
									</div>
									
									<div class="control-group">
										<label class="control-label">Tender Reference No.<span class="required">*</span></label>
										<div class="controls">
									...
									<div class="control-group">
										<label class="control-label">Tender Name<span class="required">*</span></label>
										<div class="controls">
...
function edit()
{
	...
	{	
		...
		$query=mysql_query("select * from tenders where ser='$contentid'");
...
<body onload="burstCache();" class="page-header-fixed">
	...
	<div class="page-container row-fluid">
		...
		<div class="page-content">
				...
				<div class="row-fluid">
					<div class="span12">
						...
						<ul class="breadcrumb">
							...
							<li>
								<a href="tenders.php">Pages</a>
								<span class="icon-angle-right"></span>
							</li>
							<li><a href="#">Edit Website News</a></li>
						</ul>
					</div>
				</div>
				<!-- END PAGE HEADER-->
				...
				<div class="row-fluid">
					...
					<div class="span12">
						...
						<div class="portlet box purple">
							...
							<div class="portlet-body form">
								<!-- BEGIN FORM-->
								<form action="tenders.php?action=category" enctype="multipart/form-data" id="form_page" name="editform_page" class="form-horizontal" method="post">
									...
									<div class="control-group">
										<label class="control-label">End Date</label>
										<div class="controls">
											<input class="m-wrap" size="16" type="date" name="lastdate" autocomplete="off" value="<?php echo $row_content['lastdate'];?>" id="ui_date_picker_change_year_month1"/>
										</div>
									</div>
									...
									<div class="control-group">
										<label class="control-label">Opening Date</label>
										<div class="controls">
											<input class="m-wrap" size="16" type="date" name="opdate" autocomplete="off" value="<?php echo $row_content['openingdate'];?>" id="ui_date_picker_change_year_month1"/>
										</div>
									</div>
			...
			if($row_content['extension']!='' && file_exists("../uploads/tenders/".$row_content['extension']))
			{
			?>
			<a href="../uploads/tenders/<?php echo $row_content['extension']?>" title="View Attachment" target="_blank">
			...
			?>
					&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
												...
												<?php
												if(file_exists("../uploads/tenders/".$row['extension']) && $row['extension']!='')
												{
												?>
												<b>
												
												<label onClick="var a=confirm('Are You sure delete this from database ?');if(a==true){location.href='tenders.php?action=remove_banner&ID=<?php echo $row[ser]?>&type=ext';}"><font size="2" face="Trebuchet MS" color="#FF0000">
												Remove Attachment</font></label></b>
												<?php
												}
												?>
			</p>
...
<?php
	...
	{
		echo "<script>location.href='tenders.php';</script>";
	}
}
?>
