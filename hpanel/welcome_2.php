...
if(isset($_GET['action']) && !empty($_GET['action']) && $_GET['action']=='delete_heading')
{
	...
	if($capture==$postcapture && $capture!="" && $postcapture!="")
	{
		...
		if($rs)
		{
			echo "<script>location.href='welcome_2.php?display=delete'</script>";
		}
		else
		{
			echo "<script>location.href='welcome_2.php?error=incorrect'</script>";
		}
	}
	else
	{
		echo "<p align='center'>Please Wait..</p>";
		echo "<script>location.href='welcome_2.php?status=codeinvalid'</script>";
	}
}
...
if(isset($_POST['action']) && !empty($_POST['action']) && $_POST['action']=='edit1')
{
	...
	if($capture==$postcapture && $capture!="" && $postcapture!="")
	{
		$sql_off="select * from welcome_sec where websitetype='english'";
		...
		if($row==0)
		{
			...
			if($img_response)
			{
				$query="insert into welcome_sec(websitetype,front,detail,friendly_url, otherurl,target,crdate,ipaddress,userid,heading,banner) values('$_POST[language]','$desc','$desc1','".filter_xss($_POST['friendlyurl'])."','".filter_xss($_POST['url'])."','$_POST[target]','$pdate','$_SESSION[REMOTE_ADDR]','$_SESSION[uname]','".filter_xss($_POST['txtheading'])."','$fname')";
			}
			else
			{
				$query="insert into welcome_sec(websitetype,front,detail,friendly_url, otherurl,target,crdate,ipaddress,userid,heading) values('$_POST[language]','$desc','$desc1','".filter_xss($_POST['friendlyurl'])."','".filter_xss($_POST['url'])."','$_POST[target]','$pdate','$_SESSION[REMOTE_ADDR]','$_SESSION[uname]','".filter_xss($_POST['txtheading'])."')";
			}
		...
		else
		{
			if($img_response)
			{
				$query="update welcome_sec set websitetype='$_POST[language]',front='$desc',detail='$desc1',friendly_url='".filter_xss($_POST['friendlyurl'])."',otherurl='".filter_xss($_POST['url'])."',target='$_POST[target]',modifyby='$_SESSION[uname]',heading='".filter_xss($_POST['txtheading'])."',modifydate='$pdate',ipaddress='$_SERVER[REMOTE_ADDR]',banner='$fname' ";
			}
			else
			{
				$query="update welcome_sec set websitetype='$_POST[language]',front='$desc',detail='$desc1',friendly_url='".filter_xss($_POST['friendlyurl'])."',otherurl='".filter_xss($_POST['url'])."',target='$_POST[target]',modifyby='$_SESSION[uname]',heading='".filter_xss($_POST['txtheading'])."',modifydate='$pdate',ipaddress='$_SERVER[REMOTE_ADDR]' ";
			}
		}
		$rs=mysql_query($query);
		$md5_hash = md5(rand(0,999)); 
		...
		if($rs)
		{
			echo "<script>location.href='welcome_2.php?display=update'</script>";	
		}
		else
		{
			echo "<script>location.href='welcome_2.php?error=incorrect'</script>";	
					...
					<div class="span12">
						...
						<div class="portlet box purple">
							...
							<div class="portlet-body form">
								<!-- BEGIN FORM-->
								<form action="welcome_2.php" enctype="multipart/form-data" id="form_page" name="form_page" class="form-horizontal" method="post">
