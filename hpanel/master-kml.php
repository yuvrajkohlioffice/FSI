...
if(isset($_GET['action']) && !empty($_GET['action']) && $_GET['action']=='active')
{
	...
	{	
		...
		$query="update kml_user set status='inactive' where ser='$unid_id'";
		$rs=mysql_query($query);
		
		if($rs)
		{
			echo "<script>location.href='master-kml.php?display=update'</script>";	
		}
		else
		{
			echo "<script>location.href='master-kml.php?error=incorrect'</script>";	
		}
	}
	else
	{
		echo "<p align='center'>Please Wait..</p>";
		echo "<script>location.href='master-kml.php?status=codeinvalid'</script>";
	}
}
...
if(isset($_GET['action']) && !empty($_GET['action']) && $_GET['action']=='inactive')
{
	...
	{	
		...
		$query="update kml_user set status='active' where ser='$unid_id'";
		$rs=mysql_query($query);
...
if(isset($_POST['flag']) && !empty($_POST['flag']) && $_POST['flag']==1)
{
	...
	{	
		...
		$query="insert into kml_user(state,feedbacklink,username,password,email,enctypepwd,status,crdate) values('$_POST[state]','$_POST[yesno]','$user_name','$pwd','$email','$mdpwd','active','$crdate')";
	
		$rs=mysql_query($query);
		if($rs)
		{
			echo "<script>location.href='master-kml.php?display=add'</script>";
		}
		else
		{
			echo "<script>location.href='master-kml.php?error=incorrect'</script>";
...
if(isset($_GET['action']) && !empty($_GET['action']) && $_GET['action']=='delete_heading')
{
	...
	{	
		...
		$query="delete from kml_user where ser='$unid_id'";
		$rs=mysql_query($query);
		
		if($rs)
		{
			echo "<script>location.href='master-kml.php?display=delete'</script>";
		}
		else
		{
			echo "<script>location.href='master-kml.php?error=incorrect'</script>";
...
if(isset($_POST['action']) && !empty($_POST['action']) && $_POST['action']=='edit1')
{
	...
	{	
		...
		$query="update kml_user set state='$_POST[state]',feedbacklink='$_POST[yesno]',username='$user_name',password='$pwd',email='$email',enctypepwd='$mdpwd' where ser='$unid_id'";
					...
					<div class="span12">
						...
						<div class="portlet box blue">
							<div class="portlet-title">
								<div class="caption"><i class="icon-cogs"></i>KML Users</div>
								<div class="tools">
							...
							<div class="portlet-body">
								<table class="table table-hover">
									...
									<?php
									$sno = 0;
									$res_state=mysql_query("select name,sno from state_master order by name asc");
									...
									?>										
										<tr>
											...
											<?php
												$count=0;
												$res_kmlusers=mysql_query("select ser,username,password,feedbacklink,status from kml_user where state='$row_state[sno]' order by username asc");
											...
											?>
												...
												<tr>
													...
													echo "<br><strong>Feedback Link: </strong>".$row_kmlusers['feedbacklink'];
													
													?>
													
													</td>													
													<td width="26">
													<a href="master-kml.php?action=<?php echo $row_kmlusers['status']?>&sno=<?php echo $row_kmlusers['ser']?>&log=<?php echo $_SESSION['capture_code'];?>">
													<?php
													...
													?></td>
													<td width="19">
													<a href="master-kml.php?sno=<?php echo $row_kmlusers['ser'];?>&action=<?php echo 'edit_heading'?>">
													<img border="0" src="images/edit.png" width="20" height="20" alt="Edit" name="Edit"></a></td>
													<td width="18"><input type="image" img border="0" src="images/delete.png" width="20" height="20" onclick="var a=confirm('Are You sure delete this record from database ?');if(a==true){location.href='master-kml.php?sno=<?php echo $row_kmlusers['ser']?>&action=<?php echo 'delete_heading'?>&log=<?php echo $_SESSION['capture_code'];?>';}" alt="Delete" name="delete"></td>
												</tr>
											<?php
												}
											?>
					...
					<div class="span12">
						...
						<div class="portlet box purple">
							...
							<div class="portlet-body form">
								<!-- BEGIN FORM-->
								<form action="master-kml.php?action=category" id="form_main_menu" class="form-horizontal" method="post">
...
function edit()
{
		...
		$query=mysql_query("select * from kml_user where ser='$menuid'") or die(mysql_error());
				...
				<div class="row-fluid">
					...
					<div class="span12">
						...
						<div class="portlet box purple">
							...
							<div class="portlet-body form">
								<!-- BEGIN FORM-->
								<form action="master-kml.php" id="form_main_menu" class="form-horizontal" method="post">									
									<div class="control-group">
										...
										<div class="controls">
											<select class="span6 m-wrap" name="state" size="1">
												...
												{
												?>
												<option value="<?php echo $row_state['sno'];?>" <?php if($row_content['state']==$row_state['sno']) { echo 'selected'; } ?>><?php echo $row_state['name'];?></option>
												<?php
												}
												?>
											</select>
									...
									<div class="control-group">
										...
										<div class="controls">
											<select class="span6 m-wrap" name="yesno">
												<option value="yes" <?php if($row_content['feedbacklink']=='yes') { echo 'selected'; } ?>>Yes</option>
												<option value="no" <?php if($row_content['feedbacklink']=='no') { echo 'selected'; } ?>>no</option>
											</select>
										</div>
									</div>
