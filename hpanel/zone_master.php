...
if(isset($_GET['action']) && !empty($_GET['action']) && $_GET['action']=='active')
{
	...
	{	
		$query="update  zone_master set status='inactive' where sno='$unid_id'";
		$rs=mysql_query($query);
		
		if($rs)
		{
			echo "<script>location.href='zone_master.php?display=status'</script>";	
		}
		else
		{
			echo "<script>location.href='zone_master.php?error=incorrect'</script>";	
		}
	}
	else
	{
		echo "<p align='center'>Please Wait..</p>";
		echo "<script>location.href='zone_master.php?status=codeinvalid'</script>";
	}
}
...
if(isset($_GET['action']) && !empty($_GET['action']) && $_GET['action']=='inactive')
{
	...
	if($capture==$postcapture && $capture!="" && $postcapture!="")
	{
		...
		$query="update  zone_master set status='active' where sno='$unid_id'";
		$rs=mysql_query($query);
	
		if($rs)
		{
			echo "<script>location.href='zone_master.php?display=update'</script>";	
		}
		else
		{
			echo "<script>location.href='zone_master.php?error=incorrect'</script>";	
...
if(isset($_POST['flag']) && !empty($_POST['flag']) && $_POST['flag']==1)
{
	...
	{	
		...
		$query="insert into zone_master(name,sorder,status,crdate,ipaddress)values('$title','$position','active','$crdate','$_SERVER[REMOTE_ADDR]')";
	
		$rs=mysql_query($query);		
	
		if($rs)
		{
			echo "<script>location.href='zone_master.php?display=add'</script>";
		}
		else
		{
			echo "<script>location.href='zone_master.php?error=incorrect'</script>";
...
if(isset($_GET['action']) && !empty($_GET['action']) && $_GET['action']=='delete_heading')
{
	...
	if($capture==$postcapture && $capture!="" && $postcapture!="")
	{
	    $sql_query="delete  from zone_master where sno='$unid_id'";    
	    $rs=mysql_query($sql_query);
	    
		if($rs)
		{
			echo "<script>location.href='zone_master.php?display=delete'</script>";
		}
		else
		{
			echo "<script>location.href='zone_master.php?error=incorrect'</script>";
...
if(isset($_POST['action']) && !empty($_POST['action']) && $_POST['action']=='edit1')
{
	...
	if($capture==$postcapture && $capture!="" && $postcapture!="")
	{
		...
		$sql="update zone_master set name='$title',sorder='$position' where sno='$unid_id'";	
		$rs=mysql_query($sql);
					...
					<div class="span12">
						...
						?>						<div class="portlet box blue">
							<div class="portlet-title">
								<div class="caption"><i class="icon-cogs"></i>Manage Zone Master</div>
								<div class="tools">
							...
							<div class="portlet-body">
								<table class="table table-hover">
									...
									<?php
									$sno=0;
									$res_mainmenu=mysql_query("select * from zone_master order by sorder asc");
									...
									?>
										<tr>
											...
											<td>
											<a href="zone_master.php?action=<?php echo $row_mainmenu['status']?>&sno=<?php echo $row_mainmenu['sno']?>&log=<?php echo $_SESSION['capture_code'];?>">
											<?php
											...
											<td width="35">
											<a href="zone_master.php?sno=<?php echo $row_mainmenu['sno'];?>&action=<?php echo 'edit_heading'?>&log=<?php echo $_SESSION['capture_code'];?>">
											<img border="0" src="images/edit.png" width="20" height="20" alt="Edit" name="Edit"></a></td>
											<td><input type="image" img border="0" src="images/delete.png" width="20" height="20" onclick="var a=confirm('Are You sure delete this record from database ?');if(a==true){location.href='zone_master.php?sno=<?php echo $row_mainmenu['sno']?>&action=<?php echo 'delete_heading'?>&log=<?php echo $_SESSION['capture_code'];?>';}" alt="Delete" name="delete"></td>
										</tr>
										<?php
										}
										?>
				...
				<div class="row-fluid">
					<div class="span12">
						...
						<div class="portlet box purple">
							<div class="portlet-title">
								<div class="caption"><i class="icon-reorder"></i>Add New Zone</div>
								<div class="tools">
									<a href="javascript:;" class="collapse"></a>
									<!--a href="#portlet-config" data-toggle="modal" class="config"></a-->
									<a href="javascript:;" class="reload"></a>
							...
							<div class="portlet-body form">
								<!-- BEGIN FORM-->
								<form action="zone_master.php?action=category"   id="form_main_menu" class="form-horizontal" method="post">
...
function edit()
{
	...
	{	
		...
		$query=mysql_query("select * from zone_master where sno='$menuid'");
			...
			<div class="container-fluid">
				...
				<div class="row-fluid">
					...
					<div class="span12">
						...
						<div class="portlet box purple">
							<div class="portlet-title">
								<div class="caption"><i class="icon-reorder"></i>Edit Zone - <b>"<?php echo $row_mainmenu['name'];?>"</b></div>
								<div class="tools">
									<a href="javascript:;" class="collapse"></a>
									<!--a href="#portlet-config" data-toggle="modal" class="config"></a-->
									<a href="javascript:;" class="reload"></a>
							...
							<div class="portlet-body form">
								<!-- BEGIN FORM-->
								<form action="zone_master.php"  enctype="multipart/form-data"  id="form_main_menu" class="form-horizontal" method="post">					
