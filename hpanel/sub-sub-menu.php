...
if(isset($_GET['action']) && !empty($_GET['action']) && $_GET['action']=='active')
{
	...
	if($capture==$postcapture && $capture!="" && $postcapture!="")
		...
		$query="update sub_sub_heading set status='inactive' where sno='$unid_id'";
		...
		if($rs)
		{
			echo "<script>location.href='sub-sub-menu.php?display=update'</script>";	
		}
		else
		{
			echo "<script>location.href='sub-sub-menu.php?error=incorrect'</script>";	
		}
	}
	else
	{
		echo "<p align='center'>Please Wait..</p>";
		echo "<script>location.href='sub-sub-menu.php?status=codeinvalid'</script>";
	}
}
...
if(isset($_GET['action']) && !empty($_GET['action']) && $_GET['action']=='inactive')
{
	...
	{	
		...
		$query="update sub_sub_heading set status='active' where sno='$unid_id'";
		$rs=mysql_query($query);
...
if(isset($_POST['flag']) && !empty($_POST['flag']) && $_POST['flag']==1)
{
	...
	if($capture==$postcapture && $capture!="" && $postcapture!="")
	{
		...
		$query="insert into sub_sub_heading(mainhead, heading, websitetype, subhead,pagetype,status,position,otherurl,target) values('$_POST[sno]', '$_POST[subheading]','$_POST[website_type]','$title','$_POST[typeval]','active','$position','".$url."','$_POST[target]')";
		...
		if($rs)
		{
			echo "<script>location.href='sub-sub-menu.php?display=add'</script>";
		}
		else
		{
			echo "<script>location.href='sub-sub-menu.php?error=incorrect'</script>";
...
if(isset($_GET['action']) && !empty($_GET['action']) && $_GET['action']=='delete_heading')
{
	...
	if($capture==$postcapture && $capture!="" && $postcapture!="")
	{			
		$query="delete from sub_sub_heading where sno='$unid_id'";
		$rs=mysql_query($query);
		...
		if($rs)
		{
			echo "<script>location.href='sub-sub-menu.php?display=delete'</script>";
		}
		else
		{
			echo "<script>location.href='sub-sub-menu.php?error=incorrect'</script>";
...
if(isset($_POST['action']) && !empty($_POST['action']) && $_POST['action']=='edit1')
{
	...
	{	
		...
		$query="update sub_sub_heading set mainhead='$idhead', pagetype='$typev', websitetype='$_POST[website_type]', heading='$_POST[subheading]', subhead='$title',otherurl='".$url."',target='$_POST[target]', position='$position' where sno='$unid_id'";
						...
						?>						<div class="portlet box blue">
							...
							<div class="portlet-body">
								<table class="table table-hover">
									...
									<?php
									$sno = 0;
									$res_mainmenu=mysql_query("select subhead,sno from sub_heading order by position asc");
									...
									?>										
										<tr>
											...
											<?php
												...
												$res_submenu=mysql_query("select sno,heading,subhead,otherurl,target,status from sub_sub_heading where heading='$row_mainmenu[sno]' order by subhead, position asc");
											...
											?>
												<tr>
													...
													<td width="36">
													<a href="sub-sub-menu.php?action=<?php echo $row_submenu['status']?>&sno=<?php echo $row_submenu['sno']?>&log=<?php echo $_SESSION['capture_code'];?>">
													<?php
													...
													<td width="19">
													<a href="sub-sub-menu.php?sno=<?php echo $row_submenu['sno'];?>&action=<?php echo 'edit_heading'?>">
													<img border="0" src="images/edit.png" width="20" height="20" alt="Edit" name="Edit"></a></td>
													<td width="18"><input type="image" img border="0" src="images/delete.png" width="20" height="20" onclick="var a=confirm('Are You sure delete this record from database ?');if(a==true){location.href='sub-sub-menu.php?sno=<?php echo $row_submenu['sno']?>&action=<?php echo 'delete_heading'?>&log=<?php echo $_SESSION['capture_code'];?>';}" alt="Delete" name="delete"></td>
												</tr>
											<?php
												}
											?>
				...
				<div class="row-fluid">
					...
					function valselect(theval,theform)
					{
						...
						showsubhead(theform.sno.value);					
								...
								<form action="sub-sub-menu.php?action=category" name="sub_menu" id="form_main_menu" class="form-horizontal" method="post">
...
function edit()
{
		...
		$query=mysql_query("select * from sub_sub_heading where sno='$menuid'") or die(mysql_error());
				...
				<div class="row-fluid">
					<div class="span12">
						...
						<ul class="breadcrumb">
							...
							<li>
								<a href="sub-sub-menu.php">Menu Master's</a>
								<span class="icon-angle-right"></span>
							</li>
	...
	<script type="text/javascript">
					...
					function valselect(theval,theform)
					{
						...
						showsubhead(theform.sno1.value);					
							...
							<div class="portlet-body form">
								<!-- BEGIN FORM-->
								<form action="sub-sub-menu.php" id="form_main_menu" name="edit_sub_menu" class="form-horizontal" method="post">									
									<div class="control-group">
